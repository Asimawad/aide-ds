        ...
        for i, run in enumerate(runs):
            ...
            if 'eval/is_buggy' in history.columns:
                ...
                # Aggregate exception types frequency
                 ...
                 print(f"Warning: 'eval/is_buggy' column not found in history for run '{run.name}'. Cannot calculate VCGR or exception counts for this run.")
                 pass 

            vcgr = (valid_code_count / denominator_steps_for_rates) * 100 if denominator_steps_for_rates > 0 else 0
            print(f"  Valid Code Steps: {valid_code_count}/{total_logged_steps} (using {denominator_steps_for_rates} for % calculation)")
            ...
            if 'eval/submission_produced' in history.columns:
                submission_attempt_count = (history['eval/submission_produced'] == 1).sum()
            else:
                 print(f"Warning: 'eval/submission_produced' column not found in history for run '{run.name}'. Cannot calculate CSAR for this run.")

                 pass
            csar = (submission_attempt_count / denominator_steps_for_rates) * 100 if denominator_steps_for_rates > 0 else 0
            print(f"  Submission Attempt Steps: {submission_attempt_count}/{total_logged_steps} (using {denominator_steps_for_rates} for % calculation)")
            run_csars.append(csar)


            # Average Step Execution Time
            avg_exec_time = np.nan # Default if column is missing or empty
            if 'exec/exec_time_s' in history.columns and not history['exec/exec_time_s'].empty:
                avg_exec_time = history['exec/exec_time_s'].mean()
            else:
                 print(f"Warning: 'exec/exec_time_s' column not found or empty in history for run '{run.name}'. Cannot calculate average execution time.")
            ...
            if 'code/estimated_quality' in history.columns and not history['code/estimated_quality'].empty:
                 # Ensure the column is numeric, coercing errors to NaN
                 avg_code_quality = pd.to_numeric(history['code/estimated_quality'], errors='coerce').mean()
            else:
                 print(f"Warning: 'code/estimated_quality' column not found or empty in history for run '{run.name}'. Cannot calculate average code quality.")
    ...
    #         # --- Retrieve Metrics from run.summary ---
            ...
            if steps_to_wo == float('inf'):
                ...
                run_steps_to_wo.append(inf_steps_replacement)
                 ...
                 print(f"Warning: Unexpected value for 'steps_to_first_working_code' in summary for run '{run.name}': {steps_to_wo}. Treating as infinity.")
                 runs_without_working_code += 1
                 run_steps_to_wo.append(inf_steps_replacement) # Append replacement value

            print(f"WO_{run_steps_to_wo}")

        # --- Average Metrics Across Runs (Seeds) ---
        num_valid_runs = len(runs) # Number of runs found and processed (those with history)
        ...
        avg_steps_to_wo_found_only = np.mean(steps_wo_found) if steps_wo_found else np.nan
        percentage_no_working_code = (runs_without_working_code / num_valid_runs) * 100 if num_valid_runs > 0 else 0
        ...
        results = {
            ...
            "average_metrics": {
                ...
                "Steps to First Working Code (Avg across all runs, inf replaced)": round(avg_steps_to_wo, 2) if not np.isnan(avg_steps_to_wo) else None,
                "Steps to First Working Code (Avg only for runs that found it)": round(avg_steps_to_wo_found_only, 2) if not np.isnan(avg_steps_to_wo_found_only) else None,
                "Percentage of Runs with No Working Code (%)": round(percentage_no_working_code, 2),
                "Total Run Time (Avg seconds)": round(avg_total_time, 2) if not np.isnan(avg_total_time) else None,
                "Average Step Execution Time (Avg seconds/step)": round(avg_avg_exec_time, 4) if not np.isnan(avg_avg_exec_time) else None,
                "Average LOC per Step (Avg lines)": round(avg_avg_loc, 2) if not np.isnan(avg_avg_loc) else None,
                "Average LLM Estimated Code Quality (Avg 0-10)": round(avg_avg_code_quality, 2) if not np.isnan(avg_avg_code_quality) else None,
