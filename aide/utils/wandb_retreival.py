...
def get_wb_data(
    project_name="MLE_BENCH_AIDE",
    wandb_run_name=WANDB_RUN_NAME,
    filter_pattern=FILE_FILTER_PATTERN,
    download_dir="./logs",
):
    ...
    try:
        ...
        print(
            f"Searching for run named '{WANDB_RUN_NAME}' in project '{project_path}'..."
        )
        ...
        if not runs:
            ...
            print(
                f"Error: No run found with the name '{WANDB_RUN_NAME}' in project '{project_path}'."
            )
        ...
        if len(runs) > 1:
            print(
                f"Warning: Found {len(runs)} runs with the name '{WANDB_RUN_NAME}'. Using the first one found (ID: {runs[0].id})."
            )

        # Get the run object
        found_run = runs[0]
        print(
            f"Successfully found run '{found_run.name}' ({found_run.id}). Proceeding to download files."
        )
        ...
        print("Please ensure:")
        print(
            f"- You have replaced WANDB_ENTITY, WANDB_PROJECT, and WANDB_RUN_NAME with correct values."
        )
        print(
            "- You have run 'wandb login' or set the WANDB_API_KEY environment variable."
        )
