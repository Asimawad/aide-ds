...
# Attempt to import radon, but make it optional
try:
...
try:
    ...
    wandb = None # Define to avoid UnboundLocalError

# Relative imports for when this is part of the AIDE package
try:
...
def fetch_wandb_run_data(entity: str, project: str, run_identifier: str, 
    ...
    try:
        history_df = target_run_obj.history(pandas=True)
...
if __name__ == "__main__":
    ...
    # This script should ideally not rely on a global cfg for these when run standalone.
    # The generate_all_metrics will use cfg.wandb.entity/project if loaded from the run's config.yaml.
    # For fetching, it's better to pass them directly if possible.

    final_metrics_report = generate_all_metrics(args.run_name, run_id_for_wandb=args.run_id)
    # The fetch_wandb_run_data inside generate_all_metrics will use args.entity/project if run_cfg.wandb is not available or overridden.
    # For this to work, fetch_wandb_run_data would need to accept entity/project from generate_all_metrics.
    # Current generate_all_metrics uses entity/project from loaded run_cfg or os.env.

    # For standalone, we might need to pass entity/project to generate_all_metrics if we don't want to rely on run_cfg for them.
    # However, the current structure of generate_all_metrics first loads run_cfg.
    # If run_cfg is loaded, it uses run_cfg.wandb.entity.
    # This seems okay as the config should be part of the run's logged data.
